...
std::unordered_map<int, std::list<std::pair<double,int>>> shares;
...
static int
print_element_names(xmlNode * a_node, double *share, bool co)
{
...
//                    jobs[rnum[0]][(char *)cur_node->name]=(char *)val3;
                ...
                double a;
                int d;
                sscanf(jobs[id]["DP"].c_str(),"%lf",&eps);
                
                
                
                sscanf(jobs[id]["Wsize"].c_str(),"%d",&d);
                sscanf(jobs[id]["a"].c_str(),"%lf",&a);
                //sscanf(jobs[id]["Sensitivity"].c_str(),"%lf",&sens);
                    ...
                    double sum=0.0;
                    int min=INT_MAX;
                    for (std::pair<double,int> n : shares[id]) {
                        sum+=n.first;
                        if(min>n.second)
                            min=n.second;
                    }
...
int parsemsg(char* xml, int length, double *share, bool co){
    xmlDocPtr doc;
    ...
    }else
    {
        ...
        int id=print_element_names(root_element->children, share,co);
        xmlFreeDoc(doc);
        return id;
    }
...
{   
    ...
    else
    {
        ...
        if ( bytes > 0 ){
           ...
           // printf("Array 0: %lf\n", share[0]);
            int id=parsemsg(buf, bytes, share, true);
            char vOut [12];
            snprintf(vOut, sizeof(vOut), "%u", id);
            ...
            if(share[0]==-1.0){
               ...
               // printf("d: %d\n", d);
                for(int i=0;i<id;i++){
                    ...
                    //printf("Sending share %lf to Coordinator\n", share[i]);
                    total+=reply;
        ...
        if ( bytes > 0 )
        {
            ...
            double share[1];
            share[0]=0.0;
            int id=parsemsg(buf, bytes, share, false);
            //int share=shares[id][cid].first;
            char vOut1 [12];
            snprintf(vOut1, sizeof(vOut1), "%u", id);
            char vOut2 [20];
